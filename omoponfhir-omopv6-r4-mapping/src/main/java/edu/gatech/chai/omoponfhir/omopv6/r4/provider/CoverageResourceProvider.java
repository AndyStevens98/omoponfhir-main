...
import edu.gatech.chai.omoponfhir.omopv6.r4.mapping.OmopCoverage;
...
public class CoverageResourceProvider implements IResourceProvider {
	...
	public CoverageResourceProvider() {
		...
		if (myDbType.equalsIgnoreCase("omopv5") == true) {
			myMapper = new OmopCoverage(myAppCtx);
		} else {
			myMapper = new OmopCoverage(myAppCtx);
		}
	...
	public OmopCoverage getMyMapper() {
		return myMapper;
	}
	...
	public MethodOutcome createCoverage(@ResourceParam Coverage coverage) {
		...
		Long id=null;
		try {
			id = myMapper.toDbase(coverage, null);
	...
	@Delete()
	public void deleteCoverage(@IdParam IdType theId) {
	...
	public Coverage getResourceById(@IdParam IdType theId) {
		Coverage retVal = myMapper.toFHIR(theId);
		if (retVal == null) {
			throw new ResourceNotFoundException(theId);
		}
	...
	@Search()
	public IBundleProvider findCoverageByParams(
			@OptionalParam(name = Coverage.SP_BENEFICIARY) TokenParam theBeneficiary,
			@OptionalParam(name = Coverage.SP_PATIENT) StringParam thePatient,
			
			@IncludeParam(allow={"Coverage:partof"})
			final Set<Include> theIncludes
			) {
		List<ParameterWrapper> paramList = new ArrayList<ParameterWrapper> ();

		if (theBeneficiary != null) {
			paramList.addAll(myMapper.mapParameter (Coverage.SP_BENEFICIARY, theBeneficiary, false));
		}
		if (thePatient != null) {
			paramList.addAll(myMapper.mapParameter (Coverage.SP_PATIENT, thePatient, false));
		}
	 ...
	 */
	@Update()
	public MethodOutcome updateCoverage(@IdParam IdType theId, @ResourceParam Coverage coverage) {
		validateResource(coverage);

		Long fhirId=null;
		try {
			fhirId = myMapper.toDbase(coverage, theId);
		} catch (FHIRException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void validateResource(Coverage coverage) {
	}
